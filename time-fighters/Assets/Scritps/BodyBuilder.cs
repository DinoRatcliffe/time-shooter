...
public class BodyBuilder : MonoBehaviour {
	...
	void Start () {
		...
		gameObject.transform.localScale = new Vector3(boxCollider.size.x/width, boxCollider.size.y/height, boxCollider.size.z/depth);
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				for (int r = 0; r < depth; r++) {
					...
					g.transform.localPosition = new Vector3(i-(width/2), j-(height/2), r-(depth/2));
